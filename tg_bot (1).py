...
# ==========================================
# ЗАГРУЗКА БАЗЫ ИЗ CSV ДЛЯ РУЧНОЙ СМЕНЫ ИНТЕНТА И АВТООТВЕТОВ
# ==========================================
...
async def handle_group_msg(message: Message, bot: Bot):
    ...
    # === ГЛАВНОЕ ИСПРАВЛЕНИЕ ===
    # Подтягиваем ответ не из модели (там его нет), а из загруженной базы ответов
    reply = ANSWERS_BASE.get(intent, "Шаблон ответа не найден.")
